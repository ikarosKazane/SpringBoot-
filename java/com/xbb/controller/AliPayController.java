...
import com.xbb.config.AliPayConfig;
...
public class AliPayController {
    ...
    public String goAlipay(String oid, String totalPrice, String number, HttpServletRequest request) throws Exception {
    ...
    public void alipayNotifyNotice(HttpServletRequest request, HttpServletRequest response) throws Exception {
        ...
        if (signVerified) {// 验证成功
            ...
            if (trade_status.equals("TRADE_FINISHED")) {
                ...
                orders.setNumber(out_trade_no);
                orders = ordersService.selectOne(orders);
                orders.setStatus(3);
                ordersService.updateByPrimaryKeySelective(orders);

                // ordersService.updateOrderStatusByOid(oid,uid,1);
            }
        } else {
            // 验证失败
            // log.info("支付, 验签失败...");
    ...
    public void alipayReturnNotice(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        if (signVerified) {
            ...
            //同步调用方法中返回到指定的界面
            //携带订单号并跳转到支付成功的界面
            response.sendRedirect("http://localhost:9528/#/goodsOrders");
